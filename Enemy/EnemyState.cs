...
public class IdleState : EnemyState
{
    ...
    {
        // 포탑형 탱크는 배회 중에도 포탑을 정면으로 돌려둔다.
        // 구축전차는 포탑이 없어서 배회 회전과 조준 회전을 섞지 않는다.
        if (_enemy.CanAimWhileMoving) _enemy.AimAtTarget();
...
public class CombatState : EnemyState
{
    ...
    public override void Update()
    {
        ...
        if (_attackTimer > _attackInterval)
        {
            if (_enemy.CanAttackTarget()) // 시야와 조준 조건이 맞을 때만 발사
            {
                _attackTimer = 0f;
                _attackInterval = Random.Range(_minAttackTime, _maxAttackTime);
    ...
    /// <summary>
    /// 고정형:그 자리에 멈추고 타겟을 조준
    /// 포탑형은 포탑을 돌리고, 구축전차는 AimAtTarget에서 차체를 돌린다.
    /// </summary>
    public void UpdateStationary()
    {
        _enemy.SetEngineEffect(false);
        _enemy.AimAtTarget();
        if (_enemy.UsesBodyAim == false) _enemy.RotateBodyToTarget();
    }

    /// <summary>
    /// 무시형:차체는 배회
    /// 포탑형은 이동과 조준이 분리되어 있으므로 포탑 조준을 유지한다.
    /// </summary>
    void UpdateIgnore()
    {
        _enemy.Roam();
